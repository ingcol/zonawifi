          ...
          <div class="row align-items-end">
            <div class="col-lg-8">
              <div class="page-header-title">
               <div class="d-inline">
                <h5>Informe de beneficiarios sin ayuda</h5>
              </div>
            </div>
          </div>
          <div class="col-lg-4">

            <a  @click="getExcel" v-for='policy in policies'  v-if="policy.title=='Exportar beneficiario sin ayuda' && cantidadBeneficiario>0" class="float-right btn-standar border-0 font-weight-bold text-white bg-success mt-1"><i class="ik file-text ik-file-text"></i>
            Informe en excel</a>
          </div>
...
import Loading from 'vue-loading-overlay';
  ...
  methods: {
    ...
    getExcel() {
      this.isLoading = true;
      var url = '/exportarSinAyuda';
      axios.get(url, {
        responseType: 'arraybuffer',
        params: {
         generoBeneficiario:this.generoBeneficiario,barrio_id:this.barrio_id
       }
    ...
    link.download = 'beneficiario_sin_ayuda.xlsx'
    link.click()
...
getBeneficiarios() {
this.isLoading = true;
  axios.get('/api/sinAyuda',{
   params: {
     showdata:true,generoBeneficiario:this.generoBeneficiario,barrio_id:this.barrio_id
   }
 }).then(response => {
  this.isLoading = false;
  console.log(response.data);

  this.beneficiarios = response.data.beneficiario;
